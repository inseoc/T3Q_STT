...
class KoSymSpell(SymSpell):
    ...
    def load_korean_dictionary(
        self, vocab_path, bigram_path, decompose_korean=True, load_bigrams=False, encoding="utf-8"
        # self, vocab_path, bigram_path, load_bigrams=False, encoding="utf-8"
    ):
        self._decompose_korean = decompose_korean

        if decompose_korean:
            # dict_path = "ko_50k_d.txt"
            dict_path = vocab_path.replace('txt', '_decomposed.txt')
            # bigram_path = "ko_bigrams_1M_d.txt"
            bigram_path = bigram_path.replace('txt', '_decomposed.txt')
        else:
